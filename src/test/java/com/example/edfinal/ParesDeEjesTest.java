...
class ParesDeEjesTest {
    ...
    void siempreCuatroPares() {
        for (int dim = 1; dim <= 12; dim++) {
            assertEquals(4, HelloController.paresDeVariables(dim).length, "dim=" + dim);
        }
    }
    ...
    void indicesValidos() {
        for (int dim = 1; dim <= 12; dim++) {
            for (int[] par : HelloController.paresDeVariables(dim)) {
    ...
    void cuatroVariablesCuatroParesDistintos() {
        int[][] pares = HelloController.paresDeVariables(4);
    ...
    void dosVariables() {
        for (int[] p : HelloController.paresDeVariables(2)) {
    ...
    void unaVariable() {
        for (int[] p : HelloController.paresDeVariables(1)) {
